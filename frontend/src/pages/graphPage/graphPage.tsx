...
const GraphPage = () => {
  const { data, qualityCounts, error, isConnected, outputCurrentData } =
    useMachineData();
  ...
  return (
    <div className="h-screen w-full flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-2 gap-4 p-4">
        ...
        <div className="bg-white shadow-lg rounded-lg p-4 aspect-[4/3]">
          <CustomScatter title="X축 OutputCurrent" data={outputCurrentData.X} />
        </div>

        {/* 우하단: Y축 OutputCurrent */}
        <div className="bg-white shadow-lg rounded-lg p-4 aspect-[4/3]">
          <CustomScatter title="Y축 OutputCurrent" data={outputCurrentData.Y} />
        </div>
      </div>
    </div>
  );
